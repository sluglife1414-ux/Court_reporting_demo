...
def find_audio_sec_from_transcript(item_text, segments):
    """
    Match item_text against pre-built Whisper segments to get an audio timestamp.

    Uses the same last-6-word overlap strategy as audio_validation.py so
    results are comparable. Returns the segment's start time (audio-file-relative)
    or None if no match.

    Why re-transcribe if we already have the segment text:
      The full transcript was built from 15-20 min audio chunks. A fresh 6-sec
      clip centered on the exact word gives Whisper a clean, focused input —
      less context drift, more accurate for ambiguous words.
    """
    if not item_text or len(item_text) < 6:
        return None

    words  = item_text.split()
    search = _normalize(' '.join(words[-6:]))
    ...
    if best_score < 0.4 or best_seg is None:
        return None
...
def main():
    ...
    for idx, item in enumerate(review_items, 1):
        ...
        if seek_mode == 'TRANSCRIPT':
            audio_sec = find_audio_sec_from_transcript(item['text'], segments)
        else:
