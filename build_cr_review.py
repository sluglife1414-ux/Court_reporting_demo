...
Design goal:
  ...
  Every item has a PDF page number they can jump to.
  Should be done in 15 minutes.

What we show the CR:
  A — 5 spot-check pages  (structure confirmation)
  B — Items they can fix RIGHT NOW without audio
       (Bates issues, names, missing data, exhibit gaps)
  C — Filler words  (their preference, one decision applies to all)
  D — Audio-dependent items  (count only — they handle in normal pass)
  E — Sign-off box

What we DO NOT show the CR:
  - HIGH confidence engine fixes (they don't need to review those)
  - Verify-agent internal notes they can't action without audio
  - Technical engine details

Author:  Scott + Claude
Version: 1.0  (2026-04-05)  — renamed from build_mb_review_v3.py
"""
# ──────────────────────────────────────────────────────────────
# v1.0  2026-04-05  renamed from build_mb_review_v3 — MB -> CR branding
#                   BASE = os.getcwd() fix (job folder, not engine dir)
#                   output: _CR_REVIEW.txt
# ──────────────────────────────────────────────────────────────
...
# CWD = job's work folder (set by run_pipeline.py --job-dir via os.chdir)
BASE = os.getcwd()
...
REPORTER     = caption.get('reporter_name_display', cfg.get('reporter_name', 'Court Reporter'))
...
def plain_english_label(note):
    """Convert engine-speak to plain English the CR will understand."""
    n = note.lower()
    if 'bates' in n:
...
# SECTION B -- ITEMS NEEDING THEIR ANSWER
